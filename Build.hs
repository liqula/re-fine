...
main = shakeArgs refineOptions $ do
  ...
  phony "build-frontend-prepare" $ do
    need ["build-frontend-hpack", "build-frontend-cabal2nix", "build-frontend-npm", "build-frontend-icons", "build-frontend-trans"]

  phony "build-frontend-hpack" $ do
    -- we need this goal because build.nix does not to that implicitly yet, like stack did.  we
    -- remove the cabal files first because if hpack finds them to be generated by a younger hpack,
    -- it refuses to overwrite them.
    command_ [Cwd pkgPrelude] "rm" ["-f", "refine-prelude.cabal"]
    command_ [Cwd pkgCommon] "rm" ["-f", "refine-common.cabal"]
    command_ [Cwd pkgPrelude] "hpack" []
    command_ [Cwd pkgCommon] "hpack" []

  phony "build-frontend-cabal2nix" $ do
    need ["build-frontend-hpack"]
    ...
    command_ [Cwd pkgFrontend, FileStdout $ pkgFrontend </> "default.nix"] "cabal2nix" ["--compiler", "ghcjs", "."]
