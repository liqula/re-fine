...
spec = do
  describe "VDoc" . around provideAppRunner $ do
    ...
    it "Regression test program ###" $ \(runner :: AppM DB UH Property -> IO Property) -> do
      -- pendingWith "#257"
      let program =
            [ AddVDoc (CreateVDoc (Title "title...") (Abstract "abstract...") (VDocVersion []))
            , AddEditToHead 0 sampleCreateEdit1
            ]
      monadicApp runner (runProgram program `evalStateT` initVDocs) `shouldBe` _
...
runCmd (AddEditToHead v cedit) = do
  Just vid <- gets . view $ vdocMap . at v
  cvdoc <- lift . run $ App.getCompositeVDoc vid
  let eid = cvdoc ^. compositeVDocEditID
  edit   :: Edit          <- lift . run $ App.addEdit eid cedit
  cvdoc' :: CompositeVDoc <- lift . run $ App.getCompositeVDoc vid
