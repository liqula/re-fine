...
colorName :: Maybe ST -> Color -> ST
colorName extra = (("c_" <> maybe "" (<> "_") extra) <>) . camelToUnderscore . cs . show

colorCssVar :: Color -> ST
colorCssVar c = mconcat ["$", colorName Nothing c, ": ", showRGBA $ colorValue c, ";"]

colorCssClass :: ST -> Maybe ST -> Color -> ST
colorCssClass attr extra c = mconcat [".", colorName extra c, " { ", attr, ": $", colorName Nothing c, " }"]

colorCssFile :: ST
colorCssFile = ST.unlines $ header <> (kinds <*> [minBound ..])
  where
    kinds = [colorCssVar, colorCssClass "color" Nothing, colorCssClass "background-color" (Just "bg"), colorCssClass "fill" (Just "fill")]
    header =
