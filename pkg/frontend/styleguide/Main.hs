...
colorCssClass :: Maybe ST -> Color -> ST
colorCssClass extra c = mconcat [".", colorName extra c, " { color: $", colorName extra c, " }"]

colorCssFile :: ST
colorCssFile = ST.unlines $ header <> (kinds <*> [minBound ..])
  where
    kinds = [colorCssVar, colorCssClass Nothing, colorCssClass (Just "bg"), colorCssClass (Just "fill")]
    header =
